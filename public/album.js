...
/*document.addEventListener('DOMContentLoaded', async function () {
  ...
  const albumId = urlParams.get('id');
    // Agregar un console.log para verificar el ID del álbum
...
function renderAlbumDetails(album) {
  ...
  albumTitle.textContent = album.nombreAlbum;
  
  const albumYear = document.createElement('h2');
  albumYear.textContent = `Año de lanzamiento: ${album.lanzamiento}`;

  const albumDescription = document.createElement('p');
  albumDescription.textContent = `Descripción: ${album.detalle}`;

  const albumImage = document.createElement('img');
  albumImage.src = album.imagen;
  albumImage.alt = album.nombreAlbum;
  albumImage.classList.add('albumImage');

  albumDetailsContainer.appendChild(albumTitle);
  albumDetailsContainer.appendChild(albumYear);
